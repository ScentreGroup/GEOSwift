...
private func GEOJSONCreatePolygonFromRepresentation(_ representation: NSArray) -> GEOSwift_Polygon? {
    ...
    if let coordinates = representation as? [[Double]] {
        ...
        let polygon = GEOSwift_Polygon(shell: shell, holes: nil)
        return polygon
    }
    ...
    return GEOSwift_Polygon(shell: shell, holes: rings)
}
