...
extension Geometry {
    ...
    public func mapShape() -> MKShape? {
        ...
        case is GEOSwift_Polygon:
            var exteriorRingCoordinates = (self as! GEOSwift_Polygon).exteriorRing.points.map(CLLocationCoordinate2D.init)
            let interiorRings = (self as! GEOSwift_Polygon).interiorRings.map {
        ...
        case let gc as GeometryCollection<GEOSwift_Polygon>:
            return MKShapesCollection(geometryCollection: gc)
